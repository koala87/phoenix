/**
 * Pinyin related functions implementation
 * @author Shiwei Zhang
 * @date 2014.02.11
 */
...
#pragma warning(disable:4996)	// Usage of strcpy (it is safe as hard coded code)

static std::string convert(const std::string& s, char delimiter, bool lower_letter);
/// out_buf maximum is Zhuang; char[7]
static bool search(int code, char* out_buf);

std::string yiqiding::utility::toPinyin(const std::string& s, char delimiter, bool lower_letter, CodePage::Charset encode) {
	return (encode == CodePage::GBK)?
		convert(s, delimiter, lower_letter):
		convert(transcode(s, encode, CodePage::GBK), delimiter, lower_letter);
}

/// Convert Han zi (Kanji) to Pinyin
std::string convert(const std::string& s, char delimiter, bool lower_letter) {
	unsigned char cHigh, cLow;
	int code;
	size_t len = s.size();
	char pinyin[7];
	std::string result;
	int last_done = 0;	// 0: None, 1: Unknown	2: English,	3: Chinese

	for (size_t i = 0; i < len; ++i) {
		if ((unsigned char)s[i] < 0x80) {
			if(std::isalpha(s[i])) {
				if (last_done != 0 && last_done != 2) {
		...
		cHigh = (unsigned char)s[i];  
		cLow  = (unsigned char)s[i+1];  
		if (cHigh < 0xa1 || cLow < 0xa1) {
			last_done = 1;
			continue;
		} else
			code = (cHigh - 0xa0) * 100 + cLow - 0xa0;
		++i;
		if (!search(code, pinyin)) {
			last_done = 1;
			continue;
		}

		if(lower_letter)
			for (char* p = pinyin; *p != '\0'; ++p)
				*p = tolower(*p);
...
bool search(int code, char* out_buf) { 
	switch(code) {
	case 6325:  
	case 6436:  
	case 7571:  
	...
	case 7843:  
	case 8413:  
		strcpy(out_buf,"CHUAN"); 
		break;  
	case 6675:  
		strcpy(out_buf,"CHUANG"); 
		break;  
	case 5879:  
	case 7302:  
	case 7319: 
	...
	case 6407:  
		...
		break;  
	case 8316:  
		strcpy(out_buf,"SHUAI");   
		break;  
	case 6737:  
	case 6844:  
		strcpy(out_buf,"SHUAN");   
		break;  
	case 7055:  
		strcpy(out_buf,"SHUANG");  
		break;  
	...
	case 8770:  
		...
		strcpy(out_buf,"ZHUA");  
		break;  
		strcpy(out_buf,"ZHUAI");  
		break;  
	case 6389:  
	case 6645:  
	case 8207:  
		strcpy(out_buf,"ZHUAN");  
		break;  
	case 5755:  
		strcpy(out_buf,"ZHUANG");  
		break;  
	case 6723:  
	case 7077:  
	case 7136:  
