...
//  NBS C++ Library
//
//  Last modified by Need-Being on 16/07/13.
//  Copyright (c) 2013 Neural Basis of Synesthesia. All rights reserved.
//
//  This C++ code is implemented based on md5.cc with the following header
//  comment block:
...
#include "Exception.h"
using namespace yiqiding::crypto;
...
MD5::MD5(const char* source, uint4 len) {
	init();
	update(source, len);
	finalise();
}
...
MD5::MD5(const std::vector<char>& vec) {
	init();
	update(vec);
	finalise();
}
...
void MD5::update(const uint1* input, uint4 input_length) {
	...
	if (input_length >= buffer_space) { // we have enough to fill the buffer
		// fill the rest of the buffer and transform
		memcpy (buffer + buffer_index, input, buffer_space);
		transform (buffer);
		
		// now, transform each 64-byte piece of the input,
...
void MD5::update(std::istream& in) {
	...
	while (in.good()){
		...
		update(buffer, (uint4)in.gcount());
	}
}

void MD5::update(const char* str) {
	update(str, (uint4)strlen(str));
}

void MD5::update(const std::string& str) {
	update(str.data(), (uint4)str.size());
}

void MD5::update(const std::vector<char>& v) {
	if (!v.empty())
		update(&v[0], (uint4)v.size());
}

void MD5::finalise() {
	if (finalised)
		throw Exception("md5", "already finalised", __FILE__, __LINE__);

	unsigned char bits[8];
	unsigned int index, padLen;
	static uint1 PADDING[64]= {0x80};
...
std::vector<char> MD5::vector() const {
	assert_finalised();
	return std::vector<char>(digest, digest + 16);
}

// internal functions
void MD5::assert_finalised() const {
	if (!finalised)
		throw Exception("md5", "cannot output a unfinalised digest",
				__FILE__, __LINE__);
}

void MD5::assert_not_finalised() const {
	if (finalised)
		throw Exception("md5", "cannot update a finalised digest",
				__FILE__, __LINE__);
}
